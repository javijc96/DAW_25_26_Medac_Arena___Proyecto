...
\begin{itemize}
    \item \textbf{Administrador}: usuario registrado en el sistema con permisos completos para la gestión de datos, importaciones,
    cierres de mes, configuración de tarifas, generación y envío de documentos, así como administración de usuarios y auditoría.
    \item \textbf{Usuario de consulta (usuario estándar)}: usuario registrado con permisos limitados,
    orientado a la consulta de información (guardias, vistas diaria/mensual, búsquedas y resúmenes de horas e importes cuando corresponda).
\end{itemize}
...
\begin{itemize}
    \item Consultar guardias por mes, especialidad y profesional.
    \item Visualizar la vista diaria con guardias agrupadas por especialidad, incluyendo profesional, tipo de guardia y jefe de guardia global.
    \item Buscar profesionales por nombre o identificador.
    \item Consultar el desglose de horas (según permisos establecidos).
    \item Descargar documentos disponibles (según permisos establecidos).
\end{itemize}

\subsection{Módulo del administrador}

El usuario Administrador tendrá control total de la aplicación y, además, se encargará de forma exclusiva de:
\begin{itemize}
    \item Importación de cuadrantes mensuales desde Excel, validación e informes de importación.
    \item Gestión de datos maestros (especialidades, profesionales y asociaciones).
    \item Gestión del mes de guardias (consulta, cierre y reapertura con control de permisos).
    \item Asignación automática y manual del jefe de guardia global con restricciones.
    \item Configuración de tarifas y pluses, cálculo de importes.
    \item Generación y envío por correo de documentos (PDF diario, Excel mensual).
